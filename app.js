// ==========================================================================
// 🌐 TRUSSWORK™ MASTER CORE APPLICATON TRANSACTION ENGINE (LOGICMINDS TECH)
// ==========================================================================
...
const i18n = {
    ...
    hi: { brand: "Trusswork™", kanban: "कानबान", grid: "ग्रिड", mode: "मोڈ", export: "निर्यात", signin: "साइन इन", directive: "कॉर्पोरेट निर्देश", department: "विभाग ट्रैक", deadline: "अंतिम समय सीमा", commitTask: "कार्य सौंपें", pendingExecution: "لंबित निष्पादन", activeProcessing: "سक्रिय प्रसंस्करण", completedLogs: "पूर्ण लॉग", mTotal: "कुल कार्य", mPending: "لंबित", mActive: "सक्रिय", mCompleted: "पूर्ण", mOverdue: "विलंबित", mNetwork: "नेटवर्क स्थिति", lockApp: "टर्मिनल लॉक करें" },
...
// 4. Gatekeeper Lock Screen Terminal Validation, Manual Log Out, & 3-Tier Core Licences
...
function resetInactivityTimer() {
    ...
    clearTimeout(inactivityTimeout);
    inactivityTimeout = setTimeout(() => { lockTerminal(); }, 900000); 
}
...
// 5. Global Navigation Controls Setup
...
document.getElementById('commitTaskBtn').onclick = () => {
    ...
    transaction.oncomplete = () => { loadTasks(); document.getElementById('deadlineInput').value = ''; };
};
...
function loadTasks() {
    if (!db) return;
    const lists = { pending: document.getElementById('pendingList'), active: document.getElementById('activeList'), completed: document.getElementById('completedList') };
...
function createTaskCard(task) {
    ...
    if (task.status !== 'completed') {
        ...
        else if (diff <= 24 * 60 * 60 * 1000) { bClass = 'badge-soon'; bText = 'DUE SOON'; }
    } else { bText = 'COMPLETED'; }
...
// 11. Data Stream Export Routine (Excel/CSV Pipeline)
...
function generateCSVDownloadStream(tasks) {
    if (tasks.length === 0) { alert("There are currently no tasks to export."); return; }
    ...
    virtualLink.setAttribute("download", `Trusswork_Master_Directives_Export_${new Date().toISOString().slice(0, 10)}.csv`);
